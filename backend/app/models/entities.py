...
class LearningAttempt(TimestampMixin, Base):
    ...
    word_results: Mapped[list[dict[str, Any]] | None] = mapped_column(JSONB)
...
class PointTransaction(TimestampMixin, Base):
    ...
    __table_args__ = (
        Index(
            "ix_point_transactions_profile_created",
            "profile_id",
            text("created_at DESC"),
        ),
    )

    transaction_id: Mapped[int] = mapped_column(bigint_pk, primary_key=True)
    profile_id: Mapped[int] = mapped_column(
        bigint_fk,
        ForeignKey("child_profiles.profile_id", ondelete="RESTRICT"),
        nullable=False,
    ...
    reference_id: Mapped[int | None] = mapped_column(bigint_fk)
