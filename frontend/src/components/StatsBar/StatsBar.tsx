...
const STAT_ICONS = {
  streak: { src: ICONS.fire, alt: '' },
  points: { src: ICONS.star, alt: '' },
  energy: { src: ICONS.heart, alt: '' },
} as const

function StatIcon({ type }: { type: keyof typeof STAT_ICONS }) {
  const icon = STAT_ICONS[type]
  return <img src={icon.src} alt={icon.alt} className={styles.iconImage} aria-hidden="true" />
}
...
export default function StatsBar({ stats, tone = 'light' }: Props) {
  ...
  return (
    <div className={`${styles.bar} ${tone === 'dark' ? styles.dark : ''}`} data-tour="stats">
      <button className={`${styles.stat} ${styles.streak}`} onClick={() => setShowAttendance(true)} aria-label={`Streak: ${displayStreak} days. Open attendance calendar.`}>
      ...
      <button className={`${styles.stat} ${styles.rewards}`} onClick={() => setShowPoints(true)} aria-label={`Stars: ${stats.hearts}. Open star history.`}>
      ...
      <button className={`${styles.stat} ${styles.energy}`} onClick={() => setShowEnergy(true)} aria-label={`Energy: ${energy} of ${maxEnergy}. Open energy status.`}>
